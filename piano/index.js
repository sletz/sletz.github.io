...
(async () => {
    ...
    // const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "piano", FAUST_DSP_VOICES, true);
    const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "piano", FAUST_DSP_VOICES);
