...
(async () => {
    ...
    // const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "attackey", FAUST_DSP_VOICES, true);
    const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "attackey", FAUST_DSP_VOICES);
