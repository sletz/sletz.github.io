...
(async () => {
    ...
    // const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "atomicro", FAUST_DSP_VOICES, true);
    const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "atomicro", FAUST_DSP_VOICES);
