...
(async () => {
    ...
    // const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "drone", FAUST_DSP_VOICES, true);
    const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "drone", FAUST_DSP_VOICES);
