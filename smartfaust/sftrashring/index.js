...
(async () => {
    ...
    if (audioContext.state === 'suspended') {
        ...
        overlay.addEventListener('touchstart', resumeAudioContext);

        // Optional: Remove event listeners once the context is resumed
        audioContext.onstatechange = function () {
            if (audioContext.state === 'running') {
                overlay?.remove();
                overlay.removeEventListener('click', resumeAudioContext);
                overlay.removeEventListener('touchstart', resumeAudioContext);
            }
        };
    } else {
        overlay?.remove();
