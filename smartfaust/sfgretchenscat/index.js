...
(async () => {
    ...
    // const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "sfGretchensCat", FAUST_DSP_VOICES, true);
    const { faustNode, dspMeta: { name } } = await createFaustNode(audioContext, "sfGretchensCat", FAUST_DSP_VOICES);
